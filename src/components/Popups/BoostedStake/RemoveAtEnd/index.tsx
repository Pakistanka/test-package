...
const RemoveAtEnd = (props: any) => {
    ...
    const onCancel = async () => {
        try {
            ...
            if (tryCancel) {
                setSuccess("Your stake offer will be withdrawn once the current loan ends!");
                setLoading(null);
            }
            else {
                setLoading(null)
            ...
            <div className="w-full h-full flex flex-col gap-3 items-center justify-center">
                {err ? <div className="flex flex-col items-center justify-center h-full w-full font-bold text-xs gap-2 text-center text-red-600">
                    ...
                    : success ? <Success close={props.close}>{success}</Success> :
                        <>
                            <div className="font-bold text-sm w-2/3 text-center">Are you sure you want to withdraw the stake offer at the end of current loan?</div>
